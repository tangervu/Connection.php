...
class Curl implements \Connection\Backend {
	...
	public function __construct($url, array $options = null) {
		...
		if($this->conn === false) {
			throw new \Connection\Exception("Could not connect to '$host': " . curl_error($this->conn));
		}
		...
		if(isset($this->parsedUrl['user']) && $this->parsedUrl['user']) {
			...
			if(!curl_setopt($this->conn, \CURLOPT_USERPWD, $login)) {
				throw new \Connection\Exception("Login to '$host' as '$user' failed");
			}
		}
	...
	public function get($remoteFile) {
		...
		if($data === false) {
			throw new \Connection\Exception("Could not download file '$remoteFile: " . curl_error($this->conn));
		}
	...
	public function put($data, $remoteFile) {
		...
		if(curl_exec($this->conn) === false) {
			throw new \Connection\Exception("Could not upload file '$remoteFile: " . curl_error($this->conn));
		}
	...
	public function rm($remoteFile) {
		throw new \Connection\Exception("Unable to remove files using cURL backend");
	}
	...
	public function mv($remoteFile, $newName) {
		throw new \Connection\Exception("Unable to rename files using cURL backend");
	}
	...
	public function mkdir($dirName) {
		throw new \Connection\Exception("Unable to create directories using cURL backend");
	}
	...
	public function rmdir($dirName) {
		throw new \Connection\Exception("Unable to remove directories using cURL backend");
	}
